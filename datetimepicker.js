function datetimepicker(element){
	...
	self.drawCalContainer = function(e){
        ...
        let top = ((e.target.offsetTop + e.target.clientTop) + e.target.offsetHeight) + 'px;';
        let left = ((e.target.offsetLeft + e.target.clientLeft) - 10) + 'px';
		calWrapper.style.top = top;
		calWrapper.style.left = left;
		document.body.appendChild(calWrapper);
		let wrapper = self.document('#calWrapper');
		let calContainer = '<div id="calContainer"></div>';
		wrapper.innerHTML = calContainer;
		return this;
	}
